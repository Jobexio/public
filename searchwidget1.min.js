...
if ($('.search-cta').length) {
  const a = [
      ...
      { id: 3, name: 'Åland Islands', code: 'AX' },
      { id: 4, name: 'Albania', code: 'AL' },
      { id: 5, name: 'Algeria', code: 'DZ' },
      { id: 6, name: 'American Samoa', code: 'AS' },
      { id: 7, name: 'AndorrA', code: 'AD' },
      { id: 8, name: 'Angola', code: 'AO' },
      { id: 9, name: 'Anguilla', code: 'AI' },
      { id: 10, name: 'Antarctica', code: 'AQ' },
      { id: 11, name: 'Antigua and Barbuda', code: 'AG' },
      ...
      { id: 17, name: 'Azerbaijan', code: 'AZ' },
      { id: 18, name: 'Bahamas', code: 'BS' },
      { id: 19, name: 'Bahrain', code: 'BH' },
      ...
      { id: 97, name: 'Holy See (Vatican City State)', code: 'VA' },
      ...
      { id: 117, name: "Korea, Democratic People'S Republic of", code: 'KP' },
      { id: 118, name: 'Korea, Republic of', code: 'KR' },
      ...
      { id: 194, name: 'Serbia and Montenegro', code: 'CS' },
    ...
    b = 'https://app.jobexio.com/api/website-api';
  let d = [],
    n = [],
    c = [],
    s = [],
  ...
  if (
    ...
    localStorage.getItem('w-skills'))
  )
    try {
      const l = JSON.parse(localStorage.getItem('w-skills'));
      for (const m of l) selectSkill(m);
    } catch (e) {}
  $('#country').append(
    $('<option>', { value: 'Choose country...', text: 'Choose country...' }),
  );
  for (const o of a)
    $('#country').append($('<option>', { value: o.name, text: o.name }));
  function sendEmail() {
    var e = $('#email-form #name-2').val(),
      a = $('#email-form #email-2').val(),
      i = $('#email-form #Phone-2').val();
    $.ajax({
      type: 'POST',
      url: b + '/request',
  ...
  function selectSkill(a) {
    if (!(7 < u.length)) {
      const i =
        c.find((e) => a.toLowerCase().trim() === e.toLowerCase().trim()) || a;
      $(
        `<div class="skill-item"><div class="skill-item__h">${i}</div><img loading="lazy" alt="delete" class="skill-item__close" src="https://uploads-ssl.webflow.com/5f3d775d55f59a5bb41b39e6/63875763a64c244942bb9c50_close.svg"></div>`,
      ).insertBefore('.inp-skills__wrapper .inp-skills__field-wrapper'),
        $('.inp-skills__field-wrapper input').val(''),
        $('.dd-skills').hide(),
        (c = c.filter((e) => e !== i)),
        $('.skills-descr__wrapper .skills-descr__tag').each(function () {
          $(this).text() === i.toLowerCase().trim() &&
            ($(this).remove(),
            0 === $('.skills-descr__wrapper .skills-descr__tag').length &&
              $('.eg-skills').remove());
        }),
        $('.inp-skills__field').hasClass('inp-skills__field--sm') ||
          $('.inp-skills__field').addClass('inp-skills__field--sm'),
        updateResults();
    }
  }
  function updateResults() {
    if (
      ...
      $('.skills-descr__text').hide(),
      $('.skills-descr__wrapper').hide(),
      hideResults(),
      !d.length || !n.length)
    )
      return (
        (t = !0),
        $('.loading').hide(),
        $('.skills-descr__text').css('display', 'inline-block'),
        $('.skills-descr__wrapper').css('display', 'inline-block'),
        !1
      );
    (t = !1), (r = $('#country').val());
    var e = n.find((e) => e.name === r);
    (p = $('input[type=radio][name=job-type]:checked').val() || 'freelancer'),
      (u = []),
      $('.inp-skills__wrapper .skill-item__h').each(function () {
        u.push($(this).text());
      });
    const a = [];
    for (const i of d)
      u.find((e) => e.toLowerCase().trim() === i.title.toLowerCase().trim()) &&
        a.push(i.id);
    if (!u.length)
      return (
        $('.loading').hide(),
    ...
    $('#peopleCountry').text(r),
      $.ajax({
        ...
        var { location: a, remote: e } = e;
        $('.loading').hide(),
          $('.skills-descr__text').css('display', 'inline-block'),
          $('.skills-descr__wrapper').css('display', 'inline-block'),
          2 < a + e
            ? ($('.results__cta-h').text('Hire a talent!'),
              $('#resultButton').text('Start search'),
              $('#peopleCountryNumb').text(a),
              $('#peopleRemoteNumb').text(e),
              $('.results__stats').css('display', 'flex'),
              3 < a &&
                ($('#peopleCountryPlus').text(a - 3),
                $('#resultCountryWrap .results-item__ava-more').css(
                  'display',
                  'inline-block',
                )),
              3 < e &&
                ($('#peopleRemotePlus').text(e - 3),
                $('#resultRemoteWrap .results-item__ava-more').css(
                  'display',
                  'inline-block',
                )))
            : ($('.results__cta-h').text(
                'Let us find you a perfect match for these skills',
              ),
              $('#resultButton').text('Get in touch with us')),
  ...
  function hideResults() {
    $('.results').hide(),
      $('.results__stats').hide(),
      $('#resultCountryWrap .results-item__ava-more').hide(),
      $('#resultRemoteWrap .results-item__ava-more').hide();
  }
  function saveSkills() {
    const i = [];
    $('.inp-skills__wrapper .skill-item').each(function (e, a) {
      a = $(a).find('.skill-item__h').text();
      i.push(a);
    }),
      localStorage.setItem('w-skills', JSON.stringify(i));
  }
  $('#country option').first().remove(),
    $('#country').val('Finland').change(),
    $.ajax({
      type: 'POST',
      url: b + '/skills',
      contentType: 'application/json',
    }).done((e) => {
      (d = e), (c = e.map((e) => e.title));
      try {
        for (const a of JSON.parse(localStorage.getItem('w-skills')))
          c = c.filter((e) => e !== a);
        t && updateResults();
      } catch (e) {}
    }),
    ...
    $('.inp-skills__field-wrapper input').on('input', function () {
      const a = $(this).val().toLowerCase().trim();
      (s = c.filter((e) => -1 < e.toLowerCase().indexOf(a))),
        s.length && a ? $('.dd-skills').show() : $('.dd-skills').hide(),
        $('.dd-skills').empty();
      let e = '';
      for (const i of s)
        e += `<div class="skill-item skill-item--dd"><div class="skill-item__h">${i}</div></div>`;
      $('.dd-skills').append(e);
    }),
    $('.inp-skills__field-wrapper input').on('keyup', function (e) {
      var a = $(this).val();
      13 == e.keyCode && a && selectSkill(a);
    }),
    $('#country').change(function () {
      updateResults();
    }),
    $('input[type=radio][name=job-type]').change(function () {
      updateResults();
    }),
    $('.skills-descr__wrapper .skills-descr__tag').on('click', function () {
      const a = $(this).text();
      var e = c.find((e) => e.toLowerCase().trim() === a.toLowerCase().trim());
      $(this).remove(),
        e &&
          (selectSkill(e),
          saveSkills(),
    ...
    $('.inp-skills__wrapper').on('click', '.skill-item__close', function () {
      var e = $(this).parent().find('.skill-item__h').text();
      c.push(e),
        (c = c.sort()),
        $(this).parent().remove(),
        saveSkills(),
        updateResults(),
        0 === $('.inp-skills__wrapper .skill-item').length &&
          $('.inp-skills__field').removeClass('inp-skills__field--sm');
    }),
    $('.dd-skills').on('click', '.skill-item', function () {
      selectSkill($(this).find('.skill-item__h').text()), saveSkills();
    }),
    $('#email-form').on('submit', async function (e) {
      ...
      return $('#email-form').hide(), $('.modal__success').show(), !1;
    });
}
